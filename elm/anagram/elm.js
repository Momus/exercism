...
Elm.Anagram.make = function (_elm) {
   ...
   var stringToCharDict = F2(function (inString,inDict) {
      stringToCharDict: while (true) {
         var stringTuple = $String.uncons(inString);
         var _p1 = stringTuple;
         if (_p1.ctor === "Nothing") {
               return inDict;
            } else {
               var _p3 = _p1._0._1;
               var _p2 = _p1._0._0;
               if (A2($Dict.member,_p2,inDict)) {
                     var _v2 = _p3,_v3 = A3($Dict.update,_p2,incrementEntry,inDict);
                     inString = _v2;
                     inDict = _v3;
                     continue stringToCharDict;
                  } else {
                     var _v4 = _p3,_v5 = A3($Dict.insert,_p2,1,inDict);
                     inString = _v4;
                     inDict = _v5;
                     continue stringToCharDict;
                  }
   ...
   var compareWords = F2(function (one,two) {
      var twoDict = A2(stringToCharDict,two,$Dict.empty);
      var oneDict = A2(stringToCharDict,one,$Dict.empty);
      return _U.eq($Dict.toList(oneDict),$Dict.toList(twoDict)) ? true : false;
   });
   var isAnagram = F2(function (wordOne,wordTwo) {
      var two = $String.toLower(wordTwo);
      var one = $String.toLower(wordOne);
      return _U.eq(one,two) ? false : A2(compareWords,one,two);
   });
   var detect = F2(function (word,wordList) {    return A2($List.filter,isAnagram(word),wordList);});
   return _elm.Anagram.values = {_op: _op
                                ,detect: detect
                                ,isAnagram: isAnagram
                                ,compareWords: compareWords
                                ,stringToCharDict: stringToCharDict
