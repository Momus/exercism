...
Elm.Hamming = Elm.Hamming || {};
Elm.Hamming.make = function (_elm) {
   "use strict";
   _elm.Hamming = _elm.Hamming || {};
   if (_elm.Hamming.values) return _elm.Hamming.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var _op = {};
   var distance = F2(function (strandA,strandB) {
      if (_U.eq($String.length(strandA),$String.length(strandB))) {
            var mismatch = F2(function (a,b) {    return _U.eq(a,b) ? 0 : 1;});
            var listB = $String.toList(strandB);
            var listA = $String.toList(strandA);
            return $Maybe.Just($List.sum(A3($List.map2,mismatch,listA,listB)));
         } else return $Maybe.Nothing;
   });
   return _elm.Hamming.values = {_op: _op,distance: distance};
};
