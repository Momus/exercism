...
Elm.SumOfMultiples = Elm.SumOfMultiples || {};
Elm.SumOfMultiples.make = function (_elm) {
   "use strict";
   _elm.SumOfMultiples = _elm.SumOfMultiples || {};
   if (_elm.SumOfMultiples.values) return _elm.SumOfMultiples.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var _op = {};
   var isMultiple = F2(function (big,small) {    return _U.eq(A2($Basics.rem,big,small),0) ? true : false;});
   var sOfMList = F3(function (multipleList,maxValueList,outList) {
      sOfMList: while (true) {
         var cdrMaxValue = function () {
            var _p0 = $List.tail(maxValueList);
            if (_p0.ctor === "Nothing") {
                  return _U.list([]);
               } else {
                  return _p0._0;
         ...
         var carMaxValue = $List.head(maxValueList);
         var _p1 = carMaxValue;
         if (_p1.ctor === "Nothing") {
               return outList;
            } else {
               var _p2 = _p1._0;
               var newOutList = _U.eq(A2($List.filter,isMultiple(_p2),multipleList),_U.list([])) ? outList : A2($List._op["::"],_p2,outList);
               var _v2 = multipleList,_v3 = cdrMaxValue,_v4 = newOutList;
               multipleList = _v2;
               maxValueList = _v3;
               outList = _v4;
               continue sOfMList;
   ...
   var sumOfMultiples = F2(function (multipleList,maxValue) {    return $List.sum(A3(sOfMList,multipleList,_U.range(1,maxValue - 1),_U.list([])));});
   return _elm.SumOfMultiples.values = {_op: _op,sumOfMultiples: sumOfMultiples,sOfMList: sOfMList,isMultiple: isMultiple};
};
