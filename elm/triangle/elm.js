...
Elm.Triangle.make = function (_elm) {
   ...
   var _op = {};
   var computeTriangleKind = F3(function (a,b,c) {
      return _U.eq(a,b) && _U.eq(b,c) ? $Result.Ok("equilateral") : _U.eq(a,b) || (_U.eq(a,c) || _U.eq(b,c)) ? $Result.Ok("isosceles") : $Result.Ok("scalene");
   });
   var triangleKind = F3(function (a,b,c) {
      return _U.cmp(a,0) < 1 || (_U.cmp(b,0) < 1 || _U.cmp(c,0) < 1) ? $Result.Err("Invalid lengths") : _U.cmp(2 * A2($Basics.max,a,A2($Basics.max,b,c)),
      a + (b + c)) > -1 ? $Result.Err("Violates inequality") : A3(computeTriangleKind,a,b,c);
   });
   return _elm.Triangle.values = {_op: _op,triangleKind: triangleKind,computeTriangleKind: computeTriangleKind};
};
